...
func RTTServer(args []string) error {
	...
	if len(args) > 0 {
		...
		//Check port > 1024
		if portInt < 1024 {
			log.Warn().Str(Program, Version).Msg("port < 1024, you must have root privileges")
		}
	...
	for {
		//Escucha en el puerto indicado, y bloquea a la espera
		log.Info().Msg("Waiting for clientes..")
		conn, err := listener.Accept(context.Background()) //Escuchar por nuevas conexiones
		if err != nil {
			log.Error().Msg(fmt.Sprintf("Error with new connection: %s", err.Error()))
			return err
		...
		log.Info().Msg(fmt.Sprintf("Nueva conexion %s", conn.RemoteAddr().Network()))
		go newRTTConnection(conn) //Nueva conexión de cliente
	}
